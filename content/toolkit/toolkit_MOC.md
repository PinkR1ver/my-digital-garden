---
title: Toolkit - MOC
tags:
  - MOC
  - toolkit
date: 2026-04-30
---

## Git

- [Git MOC](toolkit/git/git_MOC.md)

## Code Quality

- [Code Quality MOC](toolkit/code_quality/MOC.md)

## Shell and Terminal

- [SSH + Ghostty 剪贴板未共享排查](toolkit/ssh_ghostty_clipboard_troubleshooting.md)
- [CS2 on X11 4:3 Stretch with GNOME Top Bar](toolkit/linux_desktop/cs2_x11_4_3_stretch.md)
- [在 Ubuntu 上运行 FIFA 11：Wine、DXVK、手柄与双机 LAN](toolkit/linux_desktop/fifa11_on_ubuntu_wine.md)
- [How to deal with the error 35 by using proxy in curl](toolkit/curl/curl_set_proxy_error35.md)
- [Helix Quick Learn](toolkit/helix/helix_cli_shortcuts.md)
- [oh-my-zsh theme powerlevel10k set conda env](toolkit/zsh/oh-my-zsh/p10k/p10k_set_anaconda_env.md)

## Paperless

- [How to run paperless-ngx in windows](toolkit/paperless_ngx/How_to_run_paperless-ngx_in_windows.md)

## Writing & AI Tools

- [给 AI 生成的文档「去 AI 味」——工具与方法](toolkit/humanize_ai_writing_tools.md)

## Temporary Scripts and Study Aids

- [Common Issues in DSP Homework](tmp_script/Common_Issues_in_DSP_Homework_Script.md)
- [How to use PokerTH](tmp_script/how_to_use_PokerTH.md)
- [2023三位一体 - 生医工面试真题](tmp_script/interview_31_ans.md)
- [基于手机的头脉冲试验数据观察](tmp_script/pHIT_data_observation.md)
- [Prefix Sum](tmp_script/prefix_sum.md)
